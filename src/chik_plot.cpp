...
#include <chik/phase1.hpp>
#include <chik/phase2.hpp>
#include <chik/phase3.hpp>
#include <chik/phase4.hpp>
...
int _main(int argc, char** argv)
{
	...
	cxxopts::Options options("chik_plot",
		"Multi-threaded pipelined Chik k" + std::to_string(KMAX) + " plotter"
	...
	if(contract_addr_str.empty() && pool_key_str.empty()) {
		std::cout << "Pool Public Key (for solo farming) or Pool Contract Address (for pool farming) needs to be specified via -p or -c, see `chik_plot --help`." << std::endl;
		return -2;
	}
	if(!contract_addr_str.empty() && !pool_key_str.empty()) {
		std::cout << "Choose either Pool Public Key (for solo farming) or Pool Contract Address (for pool farming), see `chik_plot --help`." << std::endl;
		return -2;
	}
	if(farmer_key_str.empty()) {
		std::cout << "Farmer Public Key (48 bytes) needs to be specified via -f, see `chik keys show`." << std::endl;
		return -2;
	}
	if(tmp_dir.empty()) {
	...
	if(contract_addr_str.empty()) {
		...
		if(pool_key.size() != bls::G1Element::SIZE) {
			std::cout << "Invalid poolkey: " << bls::Util::HexStr(pool_key) << ", '" << pool_key_str
				<< "' (needs to be " << bls::G1Element::SIZE << " bytes, see `chik keys show`)" << std::endl;
			return -2;
		}
	}
	else {
		try {
			...
			std::cout << "Invalid contract (address): 0x"
					...
					<< "' (" << ex.what() << ", see `chik plotnft show`)" << std::endl;
			return -2;
		}
	}
	...
	{
		const std::string path = tmp_dir + ".chik_plot_tmp";
	...
	{
		const std::string path = tmp_dir2 + ".chik_plot_tmp2";
		if(auto file = FOPEN(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	{
		const std::string path = final_dir + ".chik_plot_final";
		if(auto file = FOPEN(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	
	{
		...
		for(int i = 0; i < num_files_max; ++i) {
			const std::string path = tmp_dir + ".chik_plot_tmp." + std::to_string(i);
			if(auto file = FOPEN(path.c_str(), "wb")) {
	...
	if(k < 32 && port == 8444) {
    	...
    	std::cout << "**                   !! k < 32 is not supported on chik network !!                    **" << std::endl;
	...
	std::cout << "Multi-threaded pipelined Chik k" + std::to_string(KMAX) + " plotter";
	#ifdef GIT_COMMIT_HASH
